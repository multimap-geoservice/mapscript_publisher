...
TEMP: [{},{},{}] = list templates dict
{
'type': '"pg" - postgresql, "file" - file, "var" - dict variable, necessary for all',
'name': 'variable name - necessary for type: "var" and returned pickle objects'
'path': 'filename path in json or pickle format, necessary for type: file',
'host': 'default: localhost - need for type: pg',
'port': 'default: 5432 - need for type: pg',
'user': 'necessary for type: pg',
'pass': 'necessary for type: pg',
'query': 'SQL query - return one string: json or pickle format, necessary for type: pg',
}
"""

def dict_processing(d_super, d_sub):
    """
    intheritance and update template dict
    _super - template dict or json ?
    _sub - modify template dict.
    """
    d_modify = d_super.copy()
    d_modify.update(d_sub)
    return d_modify


def dict_engine(t_down, t_up={}):
    """
    t_down - down template
    t_up - up template
    """
